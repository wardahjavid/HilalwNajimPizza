...
public final class AsciiTable {
    ...
    private static void printRow(String[] cells, int[] widths, boolean header) {
        ...
        for (int c = 0; c < widths.length; c++) {
            ...
            String fmt = numeric ? " %" + widths[c] + "s |" : " %-" + widths[c] + "s |";

            if (header) {
                System.out.printf(YELLOW + fmt + RESET, cell);
            } else if (numeric) {
                System.out.printf(GREEN + fmt + RESET, cell);
            } else {
                System.out.printf(CYAN + fmt + RESET, cell);
            }
        }
        System.out.println();
    }
